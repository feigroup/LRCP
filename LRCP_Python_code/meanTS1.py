...
def simu(mu1,m,p,R,omega,T,signal,sigma,df):
...
for Iter in range(Itermax):
    print(Iter)
    X1,Y1=simu(-1,m,p,R1,omega,T1,signal,1,df)
    X2,Y2=simu(0,m,p,R1,omega,T2,signal,1,df)
    X3,Y3=simu(1,m,p,R1,omega,T3,signal,1,df)
    X4,Y4=simu(-1,m,p,R1,omega,T4,signal,1,df)
    X5,Y5=simu(0,m,p,R1,omega,T5,signal,1,df)
...
np.save('MULmeanTcpt_num1',cpt_num)
np.save('MULmeanTd_under1',d_under)
np.save('MULmeanTd_over1',d_over)
