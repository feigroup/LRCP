...
def simu(m,p,R,omega,T,signal,mu,sigma):
    Theta  = np.reshape(np.random.uniform(0, 1, m * p), (m, p), order='F')
    ...
    # Xnew is n, m, m*p
    E=np.random.lognormal(mu,sigma,size=T*m)
...
for Iter in range(Itermax):
    print(Iter)
    X1,Y1=simu(m,p,R1,omega,T1,signal,0,1)
    X2,Y2=simu(m,p,R2,omega,T2,signal,0,1)
    X3,Y3=simu(m,p,R3,omega,T3,signal,0,1)
    X4,Y4=simu(m,p,R1,omega,T4,signal,0,1)
    X5,Y5=simu(m,p,R2,omega,T5,signal,0,1)
...
np.save('MULrankLNcpt_num1',cpt_num)
np.save('MULrankLNd_under1',d_under)
np.save('MULrankLNd_over1',d_over)
