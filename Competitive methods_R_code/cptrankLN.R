...
library("factorcpt")
###functions
simulation_setting = function(m,p,R,omega,n,signal,mu=0,sigma=1){
  Theta<-matrix(runif(m*p, min=0, max=1),m,p)
  Theta_SVD<-svd(Theta)
  ...
  E=rlnorm((n*m), meanlog = mu, sdlog = sigma)
  E=(E-mean(E))/sd(E)
  E=matrix(E,n,m)
...
for (Iter in 1:Itermax){
  ...
  X1=simulation_setting(m,p,R1,omega,T1,signal)[[1]]
  Y1=simulation_setting(m,p,R1,omega,T1,signal)[[2]]
  
  X2=simulation_setting(m,p,R2,omega,T2,signal)[[1]]
  Y2=simulation_setting(m,p,R2,omega,T2,signal)[[2]]
  
  X3=simulation_setting(m,p,R3,omega,T3,signal)[[1]]
  Y3=simulation_setting(m,p,R3,omega,T3,signal)[[2]]
  
  X4=simulation_setting(m,p,R1,omega,T4,signal)[[1]]
  Y4=simulation_setting(m,p,R1,omega,T4,signal)[[2]]
  
  X5=simulation_setting(m,p,R2,omega,T5,signal)[[1]]
  Y5=simulation_setting(m,p,R2,omega,T5,signal)[[2]]
...
save(list=ls(all=TRUE),file="cptrankLN.Rdata")