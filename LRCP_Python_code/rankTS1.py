...
def simu(m,p,R,omega,T,signal,df):
...
for Iter in range(Itermax):
    print(Iter)
    X1,Y1=simu(m,p,R1,omega,T1,signal,df)
    X2,Y2=simu(m,p,R2,omega,T2,signal,df)
    X3,Y3=simu(m,p,R3,omega,T3,signal,df)
    X4,Y4=simu(m,p,R1,omega,T4,signal,df)
    X5,Y5=simu(m,p,R2,omega,T5,signal,df)
...
np.save('MULrankTcpt_num1',cpt_num)
np.save('MULrankTd_under1',d_under)
np.save('MULrankTd_over1',d_over)
